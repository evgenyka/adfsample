...
public class adfRemoteSession {
    ...
    private String getDeviceArn(String device){
        ...
        for (Device d : listDevicesResult.getDevices())
            if (d.getFleetType().equals(FLEET_TYPE) && d.getName().equals(device)) {
                deviceArn = d.getArn();
                System.out.println(" - " + d.getName() + " (" + d.getArn() + ")");
            }
    ...
    private String createRemoteSession(String name){
    ...
    private adfRemoteSession(String device, String project, String name) throws InterruptedException {
        ...
        projectArn = getProjectArn(project);
        deviceArn = getDeviceArn(device);
        sessionArn = createRemoteSession(name);
    }
